...
/**
 * {@link #create(Class)}  创建数据表}
 * {@link #update(Object, String, String[])}  更新一些数据}
 * {@link #update(String, String, Object, String)}  更新某个字段里的数据}
 * {@link #insert(Object)}  插入数据bean(bean里自带数据)}
 * {@link #insertTransaction(List, String)}  插入大量数据}
 * {@link #get(Class)}  得到数据}
 * {@link #delete(Class, String, String...)  删除数据库某些数据}
 * {@link #clear(String)}  清除数据表的所有数据}
 * {@link #alter(Class)}  数据库添加字段(bean里自带,一般更新到新版本有用)}
 * {@link #create 创建数据表}
 */
...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void create(Class<?> cls) {  //依照bean类创建一个数据库表
    ...
    private String getTableName(Class<?> cla) {
        Annotation[] annotations = cla.getDeclaredAnnotations();    //得到类里所有的注释
        if (annotations == null || annotations.length == 0) {
            throw new IllegalStateException("you must use Table annotation for bean.Table is null");
        }
        String table = null;
        for (Annotation anno : annotations) {
            if (anno instanceof Table) {    //取table名
                table = ((Table) anno).tableName();
            }
        }
    ...
    public void create(String tableName, Class<?> cls) {
        ...
        String table = "";
        String primary = "";
        Field[] fields = cls.getDeclaredFields();   //反射获取所有字段(包含private)
        for (Field field : fields) {
            field.setAccessible(true);  //如果有private,设置可以操作的
            if (field.isAnnotationPresent(PrimaryKey.class)) {  //设置主键(自动增长)
        ...
        if (TextUtils.isEmpty(table))
            return;
        sql = sql + "(" + primary + table.substring(0, table.length() - 1) + ")";//增加)减去最后逗号
        getWritableDatabase().execSQL(sql);
    }

    private String getTypeString(Field field) { //对应SQL的类型
    ...
    public boolean update(Object obj, String tableName, String where, String[] args) {//obj是db的bean类
    ...
    public boolean insert(Object obj, String tableName) {   //插入一个bean对象(bean对象携带数据)
        if (!isExist(tableName)) {
            return false;
        }
        Class<?> cls = obj.getClass();
    ...
    public long getCount(Class<?> cls) {    //得到数据库里的数据长度
        String tableName = getTableName(cls);
        if (!isExist(tableName))
            return -1;
        SQLiteDatabase db;
    ...
    public boolean isDataExist(String tableName, String where) {    //操控的字段是否存在
        SQLiteDatabase db = getWritableDatabase();
        Cursor cursor = null;
        try {
    ...
    public boolean clear(String tableName) {    //清除数据库数据
        if (!isExist(tableName)) {
            return false;
        }
    ...
    private boolean isColumnExist(String tableName, String columnName) {    //字段是否已存在
