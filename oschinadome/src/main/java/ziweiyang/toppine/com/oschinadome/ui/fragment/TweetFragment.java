...
import ziweiyang.toppine.com.oschinadome.adapter.UserTweetAdapter;
...
import ziweiyang.toppine.com.oschinadome.other.TweetNotificationManager;
...
public class TweetFragment extends BaseGeneralRecyclerFragment<Tweet>
    ...
    public static final int CATALOG_NEW = 0X0001;   //catalog:目录
    public static final int CATALOG_HOT = 0X0002;   //用来区分那个动弹页面的显示
    ...
    public static final String CACHE_NEW_TWEET = "cache_new_tweet"; //tweet:鸣叫
    ...
    public static Fragment instantiate(long uid) {
        ...
        fragment.setArguments(bundle);  //带参给Fragment
        return fragment;
    }
    ...
    protected void initBundle(Bundle bundle) {
        ...
        switch (mReqCatalog) {
            case CATALOG_FRIENDS:   //3,4,6  都需要登录id才可以获取内容
...
//                TweetPublishService.startActionSearchFailed(AppContext.context());//TODO
            } else {
...
//                    filter.addAction(AccountBaseActivity.ACTION_ACCOUNT_FINISH_ALL);//TODO
        ...
        if (mUserId == 0 && mReqCatalog == CATALOG_MYSELF ||
                (!AccountHelper.isLogin() && mReqCatalog == CATALOG_FRIENDS)) { //点击tab 我的动弹 出现的界面
    ...
    protected void requestData() {
        ...
        switch (mReqCatalog) {
            case CATALOG_NEW:
                //注册草稿箱查询操作
//                TweetPublishService.startActionSearchFailed(AppContext.context());//TODO
                OSChinaApi.getTweetList(null, null, 1, 1, pageToken, mHandler);     //获取最新动弹数据
                break;
            case CATALOG_HOT:
                OSChinaApi.getTweetList(null, null, 1, 2, pageToken, mHandler);     //获取热门动弹数据
                break;
            case CATALOG_SOMEONE:
            case CATALOG_MYSELF:
                if (mUserId <= 0) break;
...
//                TweetPublishActivity.show(getContext(), null, "#" + tag + "#");//TODO
                break;
        }
    ...
    public void onItemClick(int position, long itemId) {
        ...
        if (tweet == null) return;
//        TweetDetailActivity.show(getContext(), tweet);//TODO
    }
...
//                        TweetPublishService.startActionDelete(getContext(), id);//TODO
                    }
                } else {
//                    LoginActivity.show(this, 1);//TODO
                }
                break;
            case R.id.bt_retry:
                //重新发送动弹
...
//                    LoginActivity.show(this, 1);//TODO
                ...
                for (String id : mPubFailedCacheIds) {
                    if (TextUtils.isEmpty(id)) continue;
//                    TweetPublishService.startActionContinue(getContext(), id);//TODO
                }
                break;
            default:
...
//                    LoginActivity.show(this, 1);//TODO
    ...
    @SuppressWarnings("unchecked")
    @Override
    protected void setListData(ResultBean<PageBean<Tweet>> resultBean) {    //设置数据源和页面刷新and下拉加载
        if (resultBean != null) {
            ...
            if (pageBean != null) {
                ...
                if (isRefreshing) {
                    AppConfig.getAppConfig(getActivity()).set("system_time", resultBean.getTime()); //记录上次刷新时间
                    mAdapter.clear();
                    ((BaseGeneralRecyclerAdapter) mAdapter).clearPreItems();
                    ((BaseGeneralRecyclerAdapter) mAdapter).addItems(items);    //自带刷新  notifyItemRangeInserted
