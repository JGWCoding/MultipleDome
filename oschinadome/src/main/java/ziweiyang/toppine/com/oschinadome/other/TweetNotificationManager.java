...
public class TweetNotificationManager {
    ...
    public static synchronized void setup(Context context) {
        ...
        if (manager.mPubTweetReceiver == null) {
            ...
            IntentFilter filter = new IntentFilter();   //注册广播 -- 添加过滤器
//            filter.addAction(TweetPublishService.ACTION_SUCCESS);//TODO 注释
...
//            filter.addAction(TweetPublishService.ACTION_RECEIVER_SEARCH_FAILED);
            appContext.registerReceiver(pubTweetReceiver, filter); //注册这个广播
    ...
    public static synchronized void destroy(Context context) {  //注销我的广播
        if (context == null)
            return;
        Context appContext = context.getApplicationContext();
        if (appContext == null)
