...
public class SolarSystemView extends View implements Runnable {
    ...
    public static final int FLUSH_RATE_LIMITATION = 16; //刷新速率极限

    private int mFlushRate = FLUSH_RATE;    //刷新速率
    private int paintCount;
    private float pivotX;       //中心点
    ...
    public SolarSystemView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTrackPaint.setStyle(Paint.Style.STROKE);   //设置画笔为描边
        mTrackPaint.setAntiAlias(true);             //设置抗锯齿

        mPlanetPaint = new Paint();
        mPlanetPaint.setStyle(Paint.Style.FILL);    //设置画笔为填充
    ...
    private ValueAnimator getAccelerateAnimator() {
        ...
        mAccelerateAnimator.setInterpolator(new DecelerateInterpolator());  //设置减速插值器
