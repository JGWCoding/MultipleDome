...
public class MainActivity extends Activity {
     ...
     */
    private ScreenRecorder mRecorder;   //屏幕录像录音对象控制器
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Utils.findEncodersByTypeAsync(VIDEO_AVC, infos -> { //找寻可以解码的编码类别
            logCodecInfos(infos, VIDEO_AVC);    //打印你手机设备的所支持的解码编码
            mAvcCodecInfos = infos;
            SpinnerAdapter codecsAdapter = createCodecsAdapter(mAvcCodecInfos);
            mVideoCodec.setAdapter(codecsAdapter);  //Video Encoder 设置选择项
            //给以前的配置进行显示
        ...
        Utils.findEncodersByTypeAsync(AUDIO_AAC, infos -> {
            ...
            mAudioCodec.setAdapter(codecsAdapter);  //录音进行配置显示
            restoreSelections(mAudioCodec, mAudioChannelCount);//显示上次配置  mAudioChannelCount为声道数量
        });
        mAudioToggle.setChecked(    //录音开关  --- 录屏是否带录音
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_MEDIA_PROJECTION) {
            ...
            File dir = getSavingDir();  //在内存卡里的Movies/ScreenCapture里
            if (!dir.exists() && !dir.mkdirs()) {   //文件夹创建不成功
                cancelRecorder();
                return;
            }
    ...
    private VideoEncodeConfig createVideoConfig() {
        final String codec = getSelectedVideoCodec();   //得到video编码设置
        if (codec == null) {
            // no selected codec ??
            return null;
        }
    ...
    private void bindViews() {
        ...
        mButton.setOnClickListener(this::onButtonClick);    //录屏按钮
